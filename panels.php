...
/**
 * Lazy panel adapters — cPanel win: only parse the panel types you actually use.
 */
if (!function_exists('rx_panel_adapter_map')) {
    function rx_panel_adapter_map(): array
    {
        return [
            'marzban' => ['Marzban.php'],
            'rebecca' => ['rebecca.php'],
            'marzneshin' => ['marzneshin.php'],
            'x-ui_single' => ['x-ui_single.php'],
            'alireza_single' => ['x-ui_single.php', 'alireza_single.php'],
            'hiddify' => ['hiddify.php'],
            'guard' => ['guard.php'],
            's_ui' => ['s_ui.php'],
            'ibsng' => ['ibsng.php'],
            'mikrotik' => ['mikrotik.php'],
            'WGDashboard' => ['WGDashboard.php'],
            // legacy / unused-on-most-hosts but keep reachable
            'alireza' => ['alireza.php'],
        ];
    }
}
...
if (!function_exists('rx_ensure_panel_adapter')) {
    function rx_ensure_panel_adapter(?string $type): void
    {
        static $loadedFiles = [];
        if ($type === null || $type === '' || $type === 'Manualsale') {
            return;
        }
        $map = rx_panel_adapter_map();
        $files = $map[$type] ?? null;
        if ($files === null) {
            return;
        }
...
if (!function_exists('rx_boot_used_panel_adapters')) {
    function rx_boot_used_panel_adapters(): void
    {
        static $booted = false;
        if ($booted) {
            return;
        ...
        if ($pdo instanceof PDO) {
            try {
                $stmt = $pdo->query('SELECT DISTINCT `type` FROM `marzban_panel`');
                if ($stmt) {
        ...
        foreach ($types as $type) {
            ...
            rx_ensure_panel_adapter($type);
        }
    }
}

rx_boot_used_panel_adapters();
...
class ManagePanel
{
    ...
    private function loadPanel(string $key, string $column = 'name_panel'): ?array
    {
        ...
        if (array_key_exists($cacheKey, self::$panelRowCache)) {
            $cached = self::$panelRowCache[$cacheKey];
            if (is_array($cached) && isset($cached['type'])) {
                rx_ensure_panel_adapter((string) $cached['type']);
            }
            return $cached;
        }
        ...
        if (is_array($value) && isset($value['type'])) {
            rx_ensure_panel_adapter((string) $value['type']);
        }
