...
function token_panelm($code_panel){
    if (isset($GLOBALS['__susanoo_panel_token_cache']['marzneshin'][$code_panel])) {
        $cached = $GLOBALS['__susanoo_panel_token_cache']['marzneshin'][$code_panel];
    ...
    if($panel['datelogin'] != null){
        ...
        if($start_date <= 3600){
            $GLOBALS['__susanoo_panel_token_cache']['marzneshin'][$code_panel] = [
                '__cached_at' => time(),
                'payload'     => $date,
            ];
            return $date;
    ...
    if (function_exists('susanoo_apply_curl_proxy')) susanoo_apply_curl_proxy($curl_token, 'panel');
