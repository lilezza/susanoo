...
function token_panel($code_panel,$verify = true){
    if ($verify && isset($GLOBALS['__susanoo_panel_token_cache']['marzban'][$code_panel])) {
        $cached = $GLOBALS['__susanoo_panel_token_cache']['marzban'][$code_panel];
    ...
    if($panel['datelogin'] != null && $verify){
        ...
        if($start_date <= 3600){
            $GLOBALS['__susanoo_panel_token_cache']['marzban'][$code_panel] = [
                '__cached_at' => time(),
                'payload'     => $date,
            ];
            return $date;
...
function getusers($location,$status)
{
    ...
    $ch = curl_init();
    if (function_exists('susanoo_apply_curl_proxy')) susanoo_apply_curl_proxy($ch, 'panel');
